...
import {
    ...
    FiCheck, FiX, FiWifi, FiWifiOff, FiEdit2
} from "react-icons/fi";

// ─── Pre-loaded crops with emojis ──────────────────────────────────────────
...
function ControlToggle({ label, icon: Icon, color, active, onToggle, disabled }) {
    return (
        ...
        ${active
                    ? `${color.bg} ${color.border} ${color.text} shadow-lg`
                    : "bg-white/40 dark:bg-slate-800/40 border-gray-200 dark:border-slate-700 text-gray-400"
                }
...
// ─── Section card ──────────────────────────────────────────────────────────
function SeccionCard({ invId, secId, sec, inv }) {
    ...
    // Load automatic mode config from Firebase in real-time
    useEffect(() => {
        ...
        <div className="bg-white/50 dark:bg-slate-800/50 rounded-2xl border border-gray-200/60 dark:border-slate-700/50 overflow-hidden">
            ...
            <div className="px-4 pb-4 pt-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                    <ControlToggle
                        ...
                        active={riego}
                        onToggle={() => toggle("riego", !riego)}
                        disabled={toggling === "riego"}
                    />
                    <ControlToggle
                        label="Malla"
                        icon={FiShield}
                        ...
                        active={malla}
                        onToggle={() => toggle("malla", !malla)}
                        disabled={toggling === "malla"}
                    />
                </div>
            </div>
            ...
            {showConfig && (
                <div className="border-t border-gray-100 dark:border-slate-700/50 px-4 py-4 space-y-5 bg-gray-50/50 dark:bg-slate-900/30">
                    ...
                    {/* Crop selector */}
                    <div>
                        <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">🌾 Cultivo en esta sección</p>
                        <button
                            onClick={() => setShowCrops(!showCrops)}
                            className="w-full flex items-center justify-between bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm text-left transition hover:border-emerald-400"
                        >
                            <span className={cultivo ? "text-gray-800 dark:text-gray-100 font-medium" : "text-gray-400"}>
                                {cultivo || "Selecciona un cultivo..."}
                            </span>
                    ...
                    <div>
                        ...
                        <p className="text-[11px] text-gray-400 mb-2">
                            Tu ESP32 debe publicar datos en:<br />
                        ...
                        <div>
                            ...
                            <div className="flex items-center justify-between bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-2xl px-4 py-3 mb-3">
                                <div>
                                    ...
                                    <p className="text-[11px] text-gray-400">El ESP32 actúa según los umbrales configurados</p>
                                </div>
                                <button
                                    onClick={() => setUmbral(["activo"], !autoConfig.activo)}
                                ...
                                <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-2xl px-4 py-3 space-y-2">
                                    ...
                                    <label className="flex items-center gap-2 text-xs text-gray-600 dark:text-gray-400">
                                        <input type="checkbox" checked={autoConfig.acciones?.riego?.bajoHumedad ?? false}
                                            onChange={(e) => setUmbral(["acciones", "riego", "bajoHumedad"], e.target.checked)}
                                            className="accent-emerald-500" />
                                        Activar riego al bajar de este umbral
                                    </label>
                                </div>
                                {/* Temperature */}
                                <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-2xl px-4 py-3 space-y-2">
                                    ...
                                    <label className="flex items-center gap-2 text-xs text-gray-600 dark:text-gray-400">
                                        <input type="checkbox" checked={autoConfig.acciones?.malla?.altaTemperatura ?? false}
                                            onChange={(e) => setUmbral(["acciones", "malla", "altaTemperatura"], e.target.checked)}
                                            className="accent-emerald-500" />
                                        Activar malla al superar este umbral
                                    </label>
                                </div>
                                {/* Radiation */}
                                <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-2xl px-4 py-3 space-y-2 sm:col-span-2">
                                    ...
                                    <label className="flex items-center gap-2 text-xs text-gray-600 dark:text-gray-400">
                                        <input type="checkbox" checked={autoConfig.acciones?.malla?.altaRadiacion ?? false}
                                            onChange={(e) => setUmbral(["acciones", "malla", "altaRadiacion"], e.target.checked)}
                                            className="accent-emerald-500" />
                            ...
                            <button onClick={guardarAuto} disabled={savingAuto}
...
// ─── Invernadero Card ──────────────────────────────────────────────────────
function InvernaderoCard({ invId, inv, isActive, onSelect, onReload }) {
    ...
    return (
        <div className={`glass rounded-3xl overflow-hidden transition-all duration-300 ${isActive ? "ring-2 ring-emerald-500/40 shadow-xl shadow-emerald-500/10" : "hover:shadow-lg"}`}>
            {/* Header */}
            <div
                className="flex items-center justify-between px-5 py-4 cursor-pointer select-none bg-white/40 dark:bg-slate-800/30"
            ...
            >
                ...
                <div className="flex items-center gap-2">
                    ...
                    {isActive && (
                        <span className="px-3 py-1.5 text-xs font-bold bg-emerald-600 text-white rounded-xl">✓ Activo</span>
                    )}
            ...
            {expanded && (
                <div className="px-4 pb-4 pt-2 space-y-3">
                    ...
                    {secs.map(([sId, sec]) => (
                        <SeccionCard key={sId} invId={invId} secId={sId} sec={sec} inv={inv} />
                    ))}
                    ...
                    {addingSection ? (
                        <div className="flex gap-2">
                            ...
                            />
                            <button onClick={crearSeccion} disabled={saving || !newSecName.trim()} className="px-3 py-2 bg-emerald-600 text-white rounded-xl text-sm font-bold hover:bg-emerald-500 transition disabled:opacity-50">
                                {saving ? "..." : <FiCheck />}
                            </button>
                            <button onClick={() => setAddingSection(false)} className="px-3 py-2 bg-gray-200 dark:bg-slate-700 rounded-xl text-gray-500 dark:text-gray-400 hover:bg-gray-300 dark:hover:bg-slate-600 transition">
                                <FiX />
                            </button>
                        </div>
                    ) : (
            ...
            {addingInv && (
                <div className="glass rounded-2xl p-5 space-y-3 animate-fadeUp border border-emerald-200/50 dark:border-emerald-800/30">
                    ...
                    <div className="flex gap-3">
                        ...
                        />
                        <button onClick={crearInvernadero} disabled={saving || !newInvName.trim()} className="px-5 py-3 bg-emerald-600 text-white rounded-xl text-sm font-bold hover:bg-emerald-500 transition disabled:opacity-50 flex items-center gap-2 shadow-lg shadow-emerald-600/20">
                            {saving ? "Creando..." : <><FiCheck /> Crear</>}
                        </button>
                        <button onClick={() => setAddingInv(false)} className="px-4 py-3 bg-gray-100 dark:bg-slate-700 text-gray-500 rounded-xl hover:bg-gray-200 dark:hover:bg-slate-600 transition">
                            <FiX />
                        </button>
                    </div>
                </div>
